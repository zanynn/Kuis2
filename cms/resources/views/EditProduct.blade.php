    ...
    <section class="single-page-title">
        <section class="contact-form ptb-50">
            <div class="container text-center">
                <h5>Edit Product {{$bodykit->title}}</h5>

                <div class="row">
                    <div class="col-md-8 col-md-offset-2 table-lol">
                        <div class="col-md-6">
                            <form action="/update/{{$bodykit->id}}" method="post" enctype="multipart/form-data">
                                {{csrf_field()}}
                                <input type="hidden" name="id" value="{{$bodykit->id}}"></br>
                                <div class="form-group"><br>
                                    <label for="title" class="zantext">Title</label>
                                    <input type="text" class="form-control" name="title" value="{{$bodykit->title}}"></br>
                                </div>
                                <div class="form-group">
                                    <label for="image" class="zantext">Feature Image</label>
                                    <div class="upload-zan"></br>
                                        <input type="file" name="image" value="{{$bodykit->imageurl}}"></br>
                                        <img class="product-img-medium" src="{{asset('storage/'.$bodykit->imageurl)}}">
                                    </div>
                                </div>
                        </div><!-- /.col-md-6 -->
                        <div class="col-md-6"><br><br>
                            <div class="form-group">
                                <label for="content" class="zantext">Content</label>
                                <textarea class="form-control textareacontent-big" name="content">{{$bodykit->content}}</textarea></br>
                            </div>
                        </div><!-- /.col-md-6 -->
                        <button type="submit" name="edit" class="btn btn-zan btn-radiuszan">SAVE</button><br><br>
                        </form>
                    </div><!-- /.col-md-8 -->
                </div>
